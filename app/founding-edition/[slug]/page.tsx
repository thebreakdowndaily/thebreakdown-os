...
export async function generateMetadata({ params }: DynamicChapterPageProps): Promise<Metadata> {
  const { slug } = await params;
  const pkg = VolumeRegistryService.getChapterBySlug(slug);

  if (!pkg) {
    return {
      title: 'Chapter Not Found — The Breakdown',
    };
  }
...
export default async function DynamicChapterPage({ params }: DynamicChapterPageProps) {
  ...
  const auditCert = GoldStandardAuditService.auditChapter1(pkg);
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-gray-100 font-sans">
      ...
      <Breadcrumbs items={[
        { label: 'Home', href: '/' },
        { label: 'Volume I', href: '/founding-edition' },
        { label: pkg.title, href: `/founding-edition/${pkg.slug}` },
      ]} />
        ...
        <header className="mb-10 border-b border-gray-800 pb-8">
          <div className="flex items-center gap-3 mb-4 flex-wrap">
            ...
            <span className="text-xs text-gray-400 font-mono">
              Evidence Score: {pkg.evidenceScore}/100 · {pkg.readingTime} min read ({pkg.wordCount.toLocaleString()} words)
            </span>
          </div>
          ...
          <p className="text-gray-300 text-lg max-w-4xl leading-relaxed">
            {pkg.sixQuestions.whatHappened.summary}
          </p>

          <div className="flex items-center gap-4 mt-6 text-xs text-gray-400 border-t border-gray-800 pt-4 flex-wrap">
            <span>Verified: <strong className="text-gray-200">{new Date(pkg.lastVerified).toLocaleDateString('en-IN', { year: 'numeric', month: 'long', day: 'numeric' })}</strong></span>
            <span>·</span>
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            <section aria-labelledby="six-questions-heading" className="space-y-8">
              ...
              <div className="bg-gray-800/40 border border-gray-700/60 rounded-xl p-6">
                <h3 className="text-xl font-bold text-amber-300 mb-3">{pkg.sixQuestions.whatHappened.title}</h3>
                <p className="text-gray-300 leading-relaxed mb-4">{pkg.sixQuestions.whatHappened.summary}</p>
                {pkg.sixQuestions.whatHappened.keyEvents && (
                  <div className="space-y-2 border-t border-gray-700/50 pt-4">
                    <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider">Chronological Milestones</h4>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      {pkg.sixQuestions.whatHappened.keyEvents.map((e) => (
                        <div key={e.year + e.event} className="bg-gray-900/60 p-2.5 rounded border border-gray-800 text-xs">
              ...
              {/* 2. Why Did It Happen */}
              <div className="bg-gray-800/40 border border-gray-700/60 rounded-xl p-6">
                <h3 className="text-xl font-bold text-amber-300 mb-3">{pkg.sixQuestions.whyDidItHappen.title}</h3>
                <p className="text-gray-300 leading-relaxed">{pkg.sixQuestions.whyDidItHappen.summary}</p>
              </div>

              {/* 3. What Alternatives Existed */}
              <div className="bg-gray-800/40 border border-gray-700/60 rounded-xl p-6">
                <h3 className="text-xl font-bold text-amber-300 mb-3">{pkg.sixQuestions.whatAlternativesEisted.title}</h3>
                <p className="text-gray-300 leading-relaxed">{pkg.sixQuestions.whatAlternativesEisted.summary}</p>
              </div>

              {/* 4. Why Strategic Autonomy */}
              <div className="bg-gray-800/40 border border-gray-700/60 rounded-xl p-6">
                <h3 className="text-xl font-bold text-amber-300 mb-3">{pkg.sixQuestions.whyStrategicAutonomy.title}</h3>
                <p className="text-gray-300 leading-relaxed">{pkg.sixQuestions.whyStrategicAutonomy.summary}</p>
              </div>

              {/* 5. Consequences */}
              <div className="bg-gray-800/40 border border-gray-700/60 rounded-xl p-6">
                <h3 className="text-xl font-bold text-amber-300 mb-3">{pkg.sixQuestions.consequences.title}</h3>
                <p className="text-gray-300 leading-relaxed">{pkg.sixQuestions.consequences.summary}</p>
              </div>

              {/* 6. Relevance Today */}
              <div className="bg-gray-800/40 border border-gray-700/60 rounded-xl p-6">
                <h3 className="text-xl font-bold text-amber-300 mb-3">{pkg.sixQuestions.relevanceToday.title}</h3>
                <p className="text-gray-300 leading-relaxed">{pkg.sixQuestions.relevanceToday.summary}</p>
              </div>
            </section>
            ...
            <section aria-labelledby="four-layer-heading" className="bg-gray-800/30 border border-amber-500/20 rounded-xl p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                <div className="bg-gray-900/60 p-4 rounded-lg border border-gray-800">
                  ...
                  <p className="text-gray-300">{pkg.fourLayers.whatHappened}</p>
                </div>
                <div className="bg-gray-900/60 p-4 rounded-lg border border-gray-800">
                  <h3 className="font-bold text-emerald-400 mb-1">Layer 2: What Evidence Shows</h3>
                  <p className="text-gray-300">{pkg.fourLayers.whatEvidenceShows}</p>
                </div>
                <div className="bg-gray-900/60 p-4 rounded-lg border border-gray-800">
                  <h3 className="font-bold text-blue-400 mb-1">Layer 3: Where Historians Disagree</h3>
                  <p className="text-gray-300">{pkg.fourLayers.whereHistoriansDisagree}</p>
                </div>
                <div className="bg-gray-900/60 p-4 rounded-lg border border-gray-800">
                  <h3 className="font-bold text-purple-400 mb-1">Layer 4: Why It Matters</h3>
                  <p className="text-gray-300">{pkg.fourLayers.whyItMatters}</p>
                </div>
              </div>
            </section>
            ...
            {pkg.fix && (
              <section aria-labelledby="linked-fix-heading" className="bg-emerald-950/20 border border-emerald-500/30 rounded-xl p-6 space-y-3">
                ...
                <h2 id="linked-fix-heading" className="text-xl font-bold text-gray-100">
                  <Link href={`/fix/${pkg.fix.slug}`} className="hover:text-emerald-300 transition-colors">
                    {pkg.fix.headline}
                  </Link>
                </h2>
                ...
                <div className="pt-2">
                  <Link href={`/fix/${pkg.fix.slug}`} className="text-xs font-bold text-amber-400 hover:text-amber-300 underline">
          ...
          <aside className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-gray-800/40 border border-gray-700/60 rounded-xl p-5 space-y-3">
              <h3 className="text-sm font-bold text-amber-300 uppercase tracking-wider border-b border-gray-700/50 pb-2">
                Level 1 Primary Sources ({(pkg.sources || []).length})
              </h3>
              <div className="space-y-2 text-xs">
                {(pkg.sources || []).map((s) => (
                  <div key={s.id || s.url} className="bg-gray-900/60 p-2.5 rounded border border-gray-800 space-y-1">
            ...
            <div className="bg-gray-800/40 border border-gray-700/60 rounded-xl p-5 space-y-3">
              <h3 className="text-sm font-bold text-amber-300 uppercase tracking-wider border-b border-gray-700/50 pb-2">
                Attested Claims ({(pkg.claims || []).length})
              </h3>
              <div className="space-y-2 text-xs">
                {(pkg.claims || []).map((c) => (
