...
const statusColors: Record<string, string> = { 
  ...
  fact_check: 'var(--color-amber-500)',
  scheduled: 'var(--color-emerald-500)',
  updated: 'var(--color-emerald-500)',
};

export default function StoryListClient({ initialStories }: { initialStories: Story[] }) {
  const [filter, setFilter] = useState<string>('all');
  const [search, setSearch] = useState('');
  
  const stories = initialStories.filter((s) => {
    ...
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '32px 24px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px' }}>
        <div>
          ...
          <p style={{ fontSize: '13px', color: 'var(--color-text-tertiary)', marginTop: '4px' }}>{initialStories.length} total</p>
        </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '11px', color: 'var(--color-text-tertiary)' }}>
                  <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: statusColors[s.status] || 'var(--color-text-tertiary)', display: 'inline-block' }} />
                  <span style={{ textTransform: 'capitalize' }}>{s.status}</span>
                  <span>·</span>
                  <span>{s.blocks?.length || 0} blocks</span>
                  <span>·</span>
