...
### 2026-08-14 — Newsroom Intelligence OS — Live Production Convergence Deliverables

- **Author:** Architecture Execution Agent (approved by Lead Systems Architect & Editor-in-Chief Joint Council)
- **Change:** Delivered the operational wiring that closes the LIVE PRODUCTION CONVERGENCE gap (report §0). (1) **Production ingestion adapter** `lib/intelligence/pib-adapter.ts` — pulls the real PIB press-releases RSS, normalizes to canonical t1 primary-source observations (NFKC + SHA-256 content hash), dedups against authoritative persisted state (idempotent pull), and emits a deterministic signal per new release; entity extraction reuses the frozen taxonomy lexicon (`getCanonicalEntityLexicon()` exported from beat-routing-service) so no taxonomy is duplicated. (2) **Cron ingestion endpoint** `POST /api/v2/newsroom/observations/pull` guarded by `x-vercel-cron` + `CRON_SECRET`; committed hourly cron in new `vercel.json`. (3) **Phase 2 authorization route** `POST /api/v2/newsroom/authorize` — managing_editor-or-above only, actor always session-derived, supports authorize/revoke; the only production mechanism for the explicit human Phase 2 decision. (4) New dependency `fast-xml-parser` (runtime, RSS parsing only). (5) Certification suite `tests/newsroom-pib-adapter.test.ts` (PIB-01…07, fixture feeds, no network). (6) Runbook `docs/newsroom/PRODUCTION_CONVERGENCE_RUNBOOK.md` incl. honest durability scope (file provider authoritative on a single persistent process; Vercel serverless requires the open Supabase repository seam for durable multi-instance state).
- **Justification:** The acceptance condition for GO-LIVE is a real production source producing a real observation that survives restart, becomes a deterministic signal/alert, reaches the correct authorized recipient, is actioned by a real user, and persists. That required an actual ingestion path (previously GET-only, no adapter) and a governed Phase 2 authorization mechanism. No new architecture: the persistence seam, taxonomy, and routing were already frozen.
- **Verification:** `npx tsc --noEmit` clean; `tests/newsroom-pib-adapter.test.ts` 7/7; full vitest suite 645/645 (638 prior + 7); smoke unchanged passing; build unchanged.


### 2026-08-14 — Newsroom Intelligence OS Production Remediation (`NEWSROOM-INTEL-PRODUCTION-REMEDIATION-01`)

- **Author:** Architecture Execution Agent (approved by Lead Systems Architect & Editor-in-Chief Joint Council)
- **Change:** Closed all 7 findings from the Final Operationalization Report audit on the Newsroom Intelligence OS (Phase 2H). Implemented: (1) durable persistence via `NewsroomStateRepository` seam — `memory` provider for dev/test, `file` provider (atomic tmp+rename, `<cwd>/data/newsroom/state.json`) for production, future Supabase provider seam left open; (2) taxonomy parity — runtime beat routing rewritten to expose exactly the frozen 16 beats with 8-pair deterministic overlap resolution; (3) actor identity — `route`/`ack` API routes derive actor from session only, removing hardcoded `actorId` and body-supplied spoofing; (4) bootstrap — `ensureNewsroomRuntime()` idempotently provisions beats + recipient registry and persists state, wired into `/newsroom`; (5) fatigue enforcement — `NEWSROOM_FATIGUE_LIMITS` (3/hr, 15/day per recipient, 5/day per beat) enforced in `routeAlert` with rolling delivered-at windows and suppression counters; (6) real metrics — core now computes window/median/reputation metrics from canonical state instead of hardcoded values; (7) recipient registry — 10 canonical recipients + demo-editor (demo mode only). Added `scripts/newsroom-smoke.ts` (`npm run smoke:newsroom`) exercising the full production boot path. Added certification suite `tests/newsroom-remediation.test.ts` (13 tests: RESTART/TAXONOMY/ROUTING/FATIGUE/IDOR/METRICS/BOOTSTRAP/DEMO).
- **Justification:** The report certified the system as "Certified & Active" while the runtime diverged from the documentation (8 beats vs 16, in-memory `Map`s as authoritative state, hardcoded actor + metrics, unenforced fatigue caps, no ingestion path). Trust requires the documentation to describe the real system. Governance and taxonomy freeze preserved — no beats added, Phase 2 still requires explicit human authorization.
- **Verification:** `npx tsc --noEmit` clean; full vitest suite 638/638 passing (incl. 27 pre-existing newsroom invariants + 13 new remediation tests); `npm run build` passes; `npm run smoke:newsroom` passes; changed files lint-clean relative to repo baseline.
