...
export default function KeyNumbersBlock({ items }: KeyNumbersData) {
  ...
  return (
    <section id="key-numbers" aria-label="Key numbers" className="py-8 sm:py-10">
      ...
      >
        {items.map((item, i) => (
          <div key={i} className="rounded-lg bg-surface-secondary border border-border p-5 sm:p-6 flex flex-col items-start text-left hover:border-brand-400/50 transition-colors overflow-hidden">
            <span className="text-4xl sm:text-5xl font-bold text-brand-400 tabular-nums leading-none tracking-tighter break-all">{item.value}</span>
