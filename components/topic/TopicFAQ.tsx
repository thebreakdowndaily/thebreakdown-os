...
export default function TopicFAQ({ questions }: { questions: FAQItem[] }) {
  const [openIndex, setOpenIndex] = useState<number | null>(null);

  if (questions.length === 0) return null;

  return (
    <section aria-label="Frequently asked questions" className="py-8 sm:py-10">
          ...
          return (
            <div key={i} className="rounded-xl bg-[#151515] border border-[#2A2A2A] overflow-hidden">
              ...
              >
                ...
                <span className={`shrink-0 text-[#D4A843] transition-transform duration-200 ${isOpen ? 'rotate-45' : ''}`}>+</span>
              </button>
              {isOpen && (
