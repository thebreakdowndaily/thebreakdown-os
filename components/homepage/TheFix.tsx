...
const TheFix: React.FC<TheFixProps> = ({ stories }) => {
  if (stories.length === 0) return null;

  return (
    <section aria-label="The Fix" className="py-8">
      <Container>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {stories.map((story) => (
          <a
            ...
            className="group bg-gray-800 border border-green-900/30 rounded-xl overflow-hidden hover:border-green-500/50 transition-colors"
          >
            {story.heroImage && (
            ...
            <div className="p-4">
              <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-500 text-white mb-2">
                Solutions
              </span>
              <h3 className="text-base font-semibold text-gray-100 group-hover:text-green-400 transition-colors mb-1 leading-snug">
                {story.headline}
              </h3>
